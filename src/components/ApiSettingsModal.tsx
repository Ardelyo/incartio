...
import { X, Key, Settings, AlertCircle, Server } from 'lucide-react';
...
export const ApiSettingsModal: React.FC<ApiSettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  }, [isOpen, store.apiProvider, store.apiKey]);

  if (!isOpen) return null;

  const handleSave = () => {
    store.setApiConfig(provider, apiKey);
    setSaved(true);
    setTimeout(() => {
      setSaved(false);
      onClose();
        ...
        >
          <div className="p-4 border-b border-[#3c4043] flex justify-between items-center bg-[#202124]">
            <h2 className="text-xl font-bold font-['Space_Grotesk'] tracking-tight flex items-center gap-2">
              <Settings size={20} className="text-[#8ab4f8]" /> API Settings
            </h2>
          ...
          <div className="p-6 space-y-5">
            <div className="bg-[#4285f4]/10 border border-[#4285f4]/30 p-3 rounded-lg flex gap-3 text-sm text-[#e8eaed]">
              ...
              <p>Connect a real-time data provider for the most accurate currency rates.</p>
            </div>

            <div className="space-y-2">
              <label className="text-xs font-bold text-[#9aa0a6] uppercase tracking-wider flex items-center gap-2">
                <Server size={14} /> Data Provider
              </label>
              <select 
                value={provider}
              ...
              >
                <option value="DEFAULT">Default (Free, No Key Required)</option>
                ...
                >
                  ...
                  <div className="text-[10px] text-[#5f6368] mt-1">
                    {provider === 'ALPHAVANTAGE' ? (
                      <a href="https://www.alphavantage.co/" target="_blank" rel="noreferrer" className="text-[#8ab4f8] hover:underline">Get Alpha Vantage Key</a>
                    ) : (
                      <a href="https://forexrateapi.com/" target="_blank" rel="noreferrer" className="text-[#8ab4f8] hover:underline">Get ForexRateAPI Key</a>
                    )}
                  </div>
                </motion.div>
              )}
